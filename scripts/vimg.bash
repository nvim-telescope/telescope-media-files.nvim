...
declare -x UEBERZUG_FIFO
UEBERZUG_FIFO="$(mktemp --dry-run --suffix "vimg-$$-ueberzug")"
...
draw_preview() {
    ...
    if [[ "$1" == "imagepreview" ]]; then
        ...
        [[ $(find "$path"/ 2>/dev/null | wc -l) -ne $frame_total ]] \
          && (mkdir -p "$path" && convert -coalesce -resize 720x480\> "$2" "$path/$file.png" & disown)

        frame_index=0
        while true; do
          frame_extracted=$(find "$path"/ 2>/dev/null| wc -l)
